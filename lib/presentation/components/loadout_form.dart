...
class LoadOutForm extends StatelessWidget {
  ...
  LoadOutForm({
    ...
    form = fb.group({
      ...
      'ervnumber': [ervnumber, Validators.required],
    }, [
      Validators.required
    ]);
  }
  ...
  void handleSubmit(Map<String, dynamic> values, BuildContext context) {
    final String ervnumber = values['ervnumber'] as String;
    final String vehicleNumber = values['vehicleNumber'] as String;
    final List<LoadOutProduct> products = (values['products'] as List<dynamic>)
        .map((e) => LoadOutProduct(
            productId: e['productId'].toString(),
            quantity: e['quantity'] as int))
        .toList();
    if (isEdit) {
      context.read<LoadOutFormCubit>().updateItem(
          loadOutId: loadOutId ?? '',
      ...
      context.read<LoadOutFormCubit>().addItem(
          ervnumber: ervnumber,
          vehicleNumber: vehicleNumber,
          products: products);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        return BlocProvider.of<LoadOutFormCubit>(context).state.status !=
            Status.loading;
      },
            ...
            children: <Widget>[
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      '${isEdit ? 'Edit' : 'Add'} Load out',
                      style: TextStyle(
                          fontSize: 20.0, fontWeight: FontWeight.w700),
                    ),
                    BlocBuilder<LoadOutFormCubit, LoadOutFormState>(
      ...
      Step(
        ...
        content: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text("ERV No: "),
            ReactiveTextField(formControlName: 'ervnumber'),
            SizedBox(height: 8),
          ],
        ),
      ),
